    ...
    <div>
      ...
      {questions.map((question, index) => (
        ...
        <div className="mt-5 ml-3 mb-3 quizQuestion p-3 w-75">
          ...
          <button onClick={displayAnswer} className="btn btn-success ml-3">Answer</button>
          {questions.length > index + 1 ? <button onClick={increaseNumber} className="btn btn-primary ml-3">
            Next Question
          </button> : null}
          
          
          {showAnswer ? <p className="mt-3">Answer: {question.answer}</p> : null}
        </div>
        : null
      ))}
    </div>
